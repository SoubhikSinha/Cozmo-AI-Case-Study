...
**`opening_widths` benchmark gate still fails** (0% within the strict 2cm tolerance) — not the same failure as before. bedroom_1's ground truth has two separate doors on the *same* wall (entrance + washroom door), which the one-opening-per-wall box model structurally cannot represent (only one opening is scanned per wall). This is a real, disclosed architectural limit of the box-room approach, not a regression — the next upgrade path is per-wall multi-opening scanning, not a threshold tweak.
