...
- Part 3 head-to-head vs. magicplan (2026.34.1) complete: `pipeline/competitor_parser.py` + `pipeline/head_to_head.py` + `scripts/run_head_to_head.py`. Real result: **0% beat-or-tie** (0/3 shared dimensions on bedroom_1 — magicplan wins length, breadth, and ceiling height, all by wide margins). `common-space` excluded (irregular ground truth, no comparable box shape). See `docs/head_to_head.md` / `docs/head_to_head_table.md`. This independently confirms the same two LiDAR issues already on record (box-fit + ceiling-trim), reinforcing them as the Part 4 fix-loop priority.
